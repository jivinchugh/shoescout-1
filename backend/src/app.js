...
function normalizeResellData(apiData) {
  ...
  brands.forEach((brand) => {
    ...
    if (Array.isArray(data)) {
      ...
      const validPrices = data.filter(
        (p) => typeof p.price === 'number' && p.size && typeof p.size === 'string'
      );
      ...
      for (const arrayKey of possibleSizeArrays) {
        if (data[arrayKey] && Array.isArray(data[arrayKey])) {
          const validPrices = data[arrayKey].filter(
            (p) => typeof p.price === 'number' && p.size && typeof p.size === 'string'
          );
        ...
        for (const arrayKey of possibleSizeArrays) {
          if (data[arrayKey] && typeof data[arrayKey] === 'object') {
            ...
            if (nestedData.sizes && Array.isArray(nestedData.sizes)) {
              const validPrices = nestedData.sizes.filter(
                (p) => typeof p.price === 'number' && p.size && typeof p.size === 'string'
              );
      ...
      if (!sizeSpecificPrices[brand]) {
        ...
        for (const field of alternativeFields) {
          if (data[field] && typeof data[field] === 'object' && data[field].sizes) {
            const validPrices = data[field].sizes.filter(
              (p) => typeof p.price === 'number' && p.size && typeof p.size === 'string'
            );
  ...
  if (
    prices.flightClub &&
    prices.flightClub.newSizes &&
    Array.isArray(prices.flightClub.newSizes)
  ) {
    console.log(
      'Processing Flight Club newSizes array with',
      prices.flightClub.newSizes.length,
      'entries'
    );
  ...
  console.log(
    ...
    Object.keys(sizeSpecificPrices).map(
      (platform) => `${platform}: ${sizeSpecificPrices[platform]?.length || 0} entries`
    )
  );
  console.log(
    '- Resell prices:',
    Object.keys(resellPrices).map((platform) => `${platform}: ${resellPrices[platform] || 'null'}`)
  );
        ...
        if (skuRes.data) {
          ...
          basicResults[0].debug_info = {
            raw_api_response_structure: {
              ...
              resellPricesKeys: skuRes.data.resellPrices
                ? Object.keys(skuRes.data.resellPrices)
                : [],
